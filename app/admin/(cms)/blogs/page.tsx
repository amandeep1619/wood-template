...
export default function BlogsListPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    const [blogsRes, catsRes] = await Promise.all([
      fetch(`/api/admin/blogs?${params}`).then((r) => r.json()),
      fetch("/api/admin/categories/blogs").then((r) => r.json()),
    ]);
    setItems(blogsRes.data ?? []);
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        ...
        <Link href="/admin/blogs/new" className="flex items-center gap-2 bg-amber-600 hover:bg-amber-700 text-white text-sm font-semibold px-4 py-2 rounded-lg transition-colors">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-100">
                ...
                <th className="text-left text-xs font-semibold text-gray-500 uppercase tracking-wider px-4 py-3 hidden lg:table-cell">Published</th>
                ...
                pagedItems.map((item) => (
                  <tr key={String(item.id)} className="hover:bg-gray-50/50 transition-colors">
                    <td className="px-5 py-3">
                      ...
                      <p className="text-xs text-gray-400">{String(item.readTime ?? 5)} min read</p>
                    </td>
                    <td className="px-4 py-3 text-gray-600 hidden sm:table-cell">{getCatName(String(item.categoryId))}</td>
                    <td className="px-4 py-3 text-gray-600 hidden md:table-cell">{String(item.authorName ?? "—")}</td>
                    <td className="px-4 py-3 text-gray-500 text-xs hidden lg:table-cell">{formatDate(item.publishedAt)}</td>
